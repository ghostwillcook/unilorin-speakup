...
/**
 * The global anonymous room.
 *
 * Identity here is a pseudonym and nothing else: PublicChatMessage carries no
 * userId by design, so this component cannot leak an author even by accident.
 * "Which bubble is mine?" is therefore answered by comparing the message
 * pseudonym against the one the server announced for this connection.
 */
...
export default function ChatPanel() {
  ...
  return (
    <GlassCard className="flex h-[34rem] max-h-[78vh] flex-col overflow-hidden">
      <PanelHeader
        title="Live Chat"
        subtitle="One anonymous room. Your real name is never shown to other students."
        right={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {pseudonym && (
              <span
                className="badge badge-neutral text-graphite"
            ...
            <EmptyState
              ...
              hint={
                online
                  ...
                  : "Messages will appear here once the chat server is running."
              }
            />
          ) : (
      ...
      <div className="border-t border-line px-5 py-4">
        {!online && (
          <div className="notice notice-warn mb-3">
            ...
            <span>
              {status === "connecting"
                ? "Connecting to the live chat server…"
                : "The live chat server is not running, so the room is read-only."}{" "}
        ...
        {draft.length > MAX_CHAT_LENGTH - 400 && (
